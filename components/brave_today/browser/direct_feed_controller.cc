...
DirectFeedController::DirectFeedController(
    ...
    : api_request_helper_(api_request_helper), prefs_(prefs) {}
...
void DirectFeedController::DownloadAllContent(GetFeedItemsCallback callback) {
  const base::DictionaryValue* direct_feeds_pref =
      prefs_->GetDictionary(prefs::kBraveTodayDirectFeeds);
  base::flat_set<GURL> direct_feed_urls;
  for (auto kv : direct_feeds_pref->DictItems()) {
    GURL url = GURL(kv.first);
    if (!url.is_valid()) {
      VLOG(1) << "Invalid direct feed url found in prefs: " << kv.first;
      continue;
    }
    direct_feed_urls.insert(url);
  }
  // Handle no direct feeds
  if (direct_feed_urls.size() == 0) {
  ...
  auto feed_content_handler = base::BarrierCallback<Articles>(
      direct_feed_urls.size(), std::move(all_done_handler));
  for (auto const& url : direct_feed_urls) {
